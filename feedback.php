           ...
           <div class="col-md-6"> 
                ...
                <?php 
                    // if(isset($_POST['button1'])){echo "<h1>string</h1>";}
                     $data=array();
   					 $sql="SELECT g_fb_good FROM live WHERE id=1;";
   					 $res2=mysqli_query($connect,$sql);
   					 while($data=mysqli_fetch_assoc($res2))
					   { 
                $last_cnt=$data['g_fb_good']; }
                ...
                if (isset($_POST['submit_good'])) 
                  { 
                    ...
                    $new_cnt=$last_cnt+1;
   					 	      $sql="UPDATE live SET g_fb_good='$new_cnt' where id=1;";
   					 	      $res=mysqli_query($connect,$sql);
   					         echo "<script>alert('Thank you for your precious feedback!');</script>";
                   
   					        // echo "<div id='testDiv' style='background:rgba(0,0,0,0.2);font-size: 60px;'>Thank You</div>";
...
<?php 
	...
	{ $last_cnt2=$data['g_fb_bad']; 
	  $lst_dustbin=$data['g_fb_dustbin'];
	  $lst_smell=$data['g_fb_smell'];
	  $lst_floor=$data['g_fb_floor'];
	  $lst_toiletpaper=$data['g_fb_toiletpaper'];
	  $lst_handwash=$data['g_fb_handwash'];
	  $lst_washbasin=$data['g_fb_washbasin'];
	}
    ...
    if(isset($_POST['submit']))
    {
    	...
    	$new_cnt2=$last_cnt2+1;
   		$sql="UPDATE live SET g_fb_bad='$new_cnt2' where id=1;";
        ...
        if(isset($_POST['dustbin']) || ($_POST['smell']) ||($_POST['floor'])|| ($_POST['paper'])|| ($_POST['handwash'])|| ($_POST['basin']))
      {
       ...
       if ($dustbin==1) {
       	...
       	$sql5="UPDATE live SET g_fb_dustbin='$new_dustbin' where id=1;";
   		$res4=mysqli_query($connect,$sql5);}

   		if ($smell==1) {
       	$new_smell=$lst_smell+$smell;
       	$sql6="UPDATE live SET g_fb_smell='$new_smell' where id=1;";
   		$res6=mysqli_query($connect,$sql6);}

   		if ($floor==1) {
       	$new_floor=$lst_floor+$floor;
       	$sql7="UPDATE live SET g_fb_floor='$new_floor' where id=1;";
   		$res7=mysqli_query($connect,$sql7);}

   		if ($paper==1) {
       	$new_toiletpaper=$lst_toiletpaper+$paper;
       	$sql8="UPDATE live SET g_fb_toiletpaper='$new_toiletpaper' where id=1;";
   		$res8=mysqli_query($connect,$sql8);}

   		if ($handwash==1) {
       	$new_handwash=$lst_handwash+$handwash;
       	$sql9="UPDATE live SET g_fb_handwash='$new_handwash' where id=1;";
   		$res9=mysqli_query($connect,$sql9);}

   		if ($basin==1) {
       	$new_washbasin=$lst_washbasin+$basin;
       	$sql11="UPDATE live SET g_fb_washbasin='$new_washbasin' where id=1;";
   		$res11=mysqli_query($connect,$sql11);}
       	
      
         // $sql="INSERT into live(dustbin,handwash,badsmell,dirtyfloor,toilletpaper,washbasin) values('$dust','$handwash','$smell','$floor','$paper','$basin');";
// g_fb_dustbin='$dust',
          //  $sql="UPDATE live set 
         	// 				  g_fb_handwash='$handwash',
          // 					     g_fb_smell='$smell',
          // 					     g_fb_floor='$floor',
          // 				   g_fb_toiletpaper='$paper',
          // 					 g_fb_washbasin='$basin' where id=1;";
          
          // $res5=mysqli_query($connect,$sql);
      }
